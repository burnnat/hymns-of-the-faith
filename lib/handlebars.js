...
const findTags = (text, tag) => text.match(new RegExp(`<${tag}>.*?<\\/${tag}>`, 'g'));

Handlebars.registerHelper('translationTable', function(options) {
	let raw = options.fn(this);
	let translations = findTags(raw, 'tr');

	let headers = translations.map((text) => findTags(text, 'th').join(''));
	let bodies = translations.map((text) => findTags(text, 'td'))

	let stanzas = [];
	let stanzaCount = bodies.reduce((count, body) => Math.max(count, body.length), 0);

	for (let i = 0; i < stanzaCount; i++) {
		stanzas[i] = bodies.map((body) => body[i]);
	}
	...
	return new Handlebars.SafeString(
		'<table>' +
			...
			'</thead>' +
			'<tbody>' +
				stanzas.map((stanza) => '<tr>' + stanza.join('') + '</tr>').join('') +
			'</tbody>' +
		'</table>'
	);
});

Handlebars.registerHelper('translation', function(header, options) {
	let raw = options.fn(this);
	let stanzas = raw.replace(/^[ \t]+/gm, '').split('\n\n');

	return new Handlebars.SafeString(
		'<tr>' +
			'<th>' + header + '</th>' +
			stanzas.map((text) => '<td>' + text.trim().replace(/\n/g, '<br/>') + '</td>').join('') +
		'</tr>'
	);
});
